 ...
 * /spp/spp_city/{kabkot}:
 *   get:
 *     summary: Fetch SPP city results by kabkot_id
 *     description: Retrieves SPP city result data filtered by kabkot_id, ordered by capaian in ascending order.
 *     tags:
 *       - SPP Indikator
 *     parameters:
 *       - in: path
 ...
 *     responses:
 *       200:
 *         description: Successfully retrieved SPP city results.
 *         content:
 *           application/json:
 *             schema:
 *               type: object
 ...
 *                       # Add other properties returned in spp_city_result if necessary
 *       400:
...
async function getSppCity(kabkot_id?: string) {
    ...
    const data = await prisma.spp_city_result.findMany(query);
